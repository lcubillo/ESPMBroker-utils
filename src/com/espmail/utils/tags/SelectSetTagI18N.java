...
import com.espmail.modelo.maestras.CategoriasRedes;
import com.espmail.modelo.maestras.I18N;
...
public class SelectSetTagI18N extends BodyTagSupport{
	...
	//valor de la internacionalizacion
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			try {
				...
				itrCol = SetUtils.values(Class.forName(this.set)).iterator();
			} catch (ClassNotFoundException e) {		
				throw new JspException("error en la asignacion de la clase."+property);
			}
			if(labelKey!=null)
			{
				while(itr.hasNext() && itrCol.hasNext())
				{					
					out.write("<option value=\"");
					
					
					String valor= itr.next().toString();
					String valori18n =   ((I18N) itrCol.next()).toI18N(valor);
					out.write(valor+"\"");
					if(isSelected(valor,match))
						out.write(" selected=\"selected\"");
					out.write(">"+LocaleSupport.getLocalizedMessage(this.pageContext, labelKey+valori18n));
					out.write("</option>\n");
				}
			}
